...
<xsl:stylesheet
	...
	<xsl:template name="element-simpleContent-forwardee" match="xsl:template[@name = 'element-simpleContent-forwardee']">
		<xsl:param name="root-document" /> <!-- contains root document -->
		<xsl:param name="root-path" /> <!-- contains path from root to included and imported documents -->
		<xsl:param name="root-namespaces" /> <!-- contains root document's namespaces and prefixes -->
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:call-template name="log">
					<xsl:with-param name="reference">xs:element[xs:complexType/xs:simpleContent]-forwardee</xsl:with-param>
				</xsl:call-template>
