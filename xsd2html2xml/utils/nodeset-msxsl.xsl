...
<xsl:stylesheet
	...
	<xsl:template name="forward">
		...
		<xsl:call-template name="log">
			<xsl:with-param name="reference">fwd (MSXSL): <xsl:value-of select="$template" /></xsl:with-param>
		</xsl:call-template>
		
		<!-- use MSXSL node-set extension for namespace-documents -->
		<xsl:apply-templates select="$stylesheet/*/xsl:template[@name=$template]">
			...
			<xsl:with-param name="namespace-documents" select="msxsl:node-set($namespace-documents)" />
	...
	<xsl:template name="forward-root">
		...
		<xsl:call-template name="log">
			<xsl:with-param name="reference">fwd-root (MSXSL): <xsl:value-of select="$template" /></xsl:with-param>
		</xsl:call-template>
		
		<!-- use MSXSL node-set extension for root-namespaces -->
		<xsl:apply-templates select="$stylesheet/*/xsl:template[@name=$template]">
			<!-- calling node is provided as parameter $node -->
			<xsl:with-param name="node" select="." />
			
			<!-- root namespaces are forwarded as nodeset -->
			<xsl:with-param name="root-namespaces" select="msxsl:node-set($root-namespaces)" />
